"""Seed demo data: knowledge base, documents, SQL demo data."""
...
def seed_documents():
    """Create demo knowledge base and ingest documents."""
    db = SessionLocal()
        ...
        try:
            ...
            for chunk in chunks:
                db.add(DocumentChunk(
                    ...
                    vector_id=None,  # Will be set when Milvus is available
                ))
    ...
    kb.document_count = len(list(DOCUMENTS_DIR.glob("*.md")))
    kb.chunk_count = total_chunks
    db.commit()
    db.close()
    print(f"Seeded {kb.document_count} documents, {total_chunks} chunks")
    return kb.id


def seed_sql_data():
    """Seed demo SQL data: products, customers, sales orders, campaigns."""
    db = SessionLocal()
